...
def build(root,version,close_main):
    ...
    def button(parent,text,primary=False,compact=False):
        return appearance.bind(RoundedButton(parent,text=text,font=(FONT_FAMILY,10),bg=colors['blue' if primary else 'field'],fg=colors['white' if primary else 'text'],padx=12,pady=6 if compact else 10),bg='blue' if primary else 'field',fg='white' if primary else 'text')
    ...
    def saved(plan):
        ...
        preview.configure(text=(content[:180]+('…' if len(content)>180 else '')) or ('包含图片或文件附件' if pending else '保存后，后续要求会在原任务满足投递条件时发送。'))
