...
- 独立后续任务卡片显示已保存内容，突出添加 / 编辑操作；整理原生标题栏、按钮层级和底部工具栏。
- macOS 关闭窗口保留后台；菜单栏可恢复窗口。退出程序会通过已有暂停接口暂停后续自动检查，正在执行的任务不被取消；菜单保留“仅退出界面”选项。
- 编辑器统一附件入口，保留图片缩略图、普通文件和逐个移除，附件区域限制高度并滚动。
- 继续使用已有监控、保存、延迟发送、附件数据格式和菜单栏恢复逻辑；Apple Silicon / Intel 构建流程保留。
...
Windows 主界面与输入框采用统一深色圆角外观、标题字号和任务菜单宽度，可拖动标题栏或空白边缘移动窗口。绿色圆点代表主备监控均已启用，红色代表未全部启用或无法确认；状态每五秒读取实际计划任务。
